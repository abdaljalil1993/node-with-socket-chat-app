...
function addmsg(app,sende,reciver,msg,t,dt,state,message_id){ 
        const sender = sende;
        const rec = reciver;
        const message = msg;
        const type = t;
        ...
        const dt1=dt;
          ...
          db.query("select * from chat_room where (sender_id=? and reciver_id=?)or(sender_id=? and reciver_id=?)",[sender,rec,rec,sender],(er2,result2)=>{
            if(er2) throw(er2)
              ...
              if(result2.length >0 )
              {
                  db.query('insert into messages(sender_id,reciver_id,message_type,message,created_at,state,message_id) values(?,?,?,?,?,?,?)', [sender,rec,type,message,dt1,state1,msg_id], (err, results1, fields)=> {
              if (err) throw (err)
              ...
              //resolve(results)
                ...
                db.query("insert into chat_room(sender_id,reciver_id,state) values(?,?,?)",[sender,rec,0],(er3,res3)=>{
                    if(!er3)
                    {
                      db.query('insert into messages(sender_id,reciver_id,message_type,message,created_at,state,message_id) values(?,?,?,?,?,?,?)', [sender,rec,type,message,dt1,state1,msg_id], (err2, results, fields)=> {
                        if (err2) throw (err2)
                        //console.log('resultes : ',results.insertId)
                        //resolve(results)
                        else{
