...
function addmsg(app,sende,reciver,msg,t,dt,state){ 
      ...
      //create new chat for secret message 
          ...
          db.query("select * from chat_room where (sender_id=? and reciver_id=?)or(sender_id=? and reciver_id=?)",[sender,rec,rec,sender],(er2,result2)=>{
            if(er2) throw(er2)
              ...
              if(result2.length >0 )
              {
                  db.query('insert into messages(sender_id,reciver_id,message_type,message,created_at,state,is_secret) values(?,?,?,?,?,?,?)', [sender,rec,type,message,dt1,dt1,state1,true], (err, results1, fields)=> {
              if (err) throw (err)
                ...
                db.query("insert into chat_room(sender_id,reciver_id,state) values(?,?,?)",[sender,rec,0],(er3,res3)=>{
                    if(!er3)
                    {
                      db.query('insert into messages(sender_id,reciver_id,message_type,message,created_at,state,is_secret) values(?,?,?,?,?,?,?)', [sender,rec,type,message,dt1,dt1,state1,true], (err2, results, fields)=> {
                        if (err2) throw (err2)
